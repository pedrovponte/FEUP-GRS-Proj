...
$CONNECTION loadBalancer 1.1.1.1; assertTrue
$CONNECTION loadBalancer google.com; assertTrue

# Stop VPN from external_host
docker exec external_host /bin/bash -c "pkill -f openvpn"
...
# Connect external_host to VPN
docker cp ~/usr1.ovpn external_host:/etc/openvpn/usr1.ovpn
docker exec external_host /bin/bash -c "openvpn --config /etc/openvpn/usr1.ovpn --askpass <(echo '12345')" > /dev/null &
sleep 2
